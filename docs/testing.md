...
broken/duplicate release: **static checks → executed smoke → unit math → deploy gates →
runtime throttling.** Each layer catches a different class of failure.
...
```
Exit code is non-zero on any failure. Current baseline: **18 checks, all green.**
...
### Layer 2 — Structure (static cross-checks between `index.html` and `app.js`, 6 checks)
...
## 2. Standing rule — verify after every major commit
...
## 3. Deploy safety gates — the `deploy` skill
...
## 4. How the deploy skill was hardened (eval loop)
...
## 5. Runtime backpressure inside the app
